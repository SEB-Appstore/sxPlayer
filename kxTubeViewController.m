...
NSString * const KxTMovieParameterMinBufferedDuration = @"KxMovieParameterMinBufferedDuration";
NSString * const KxTMovieParameterMaxBufferedDuration = @"KxMovieParameterMaxBufferedDuration";
NSString * const KxTMovieParameterDisableDeinterlacing = @"KxMovieParameterDisableDeinterlacing";

////////////////////////////////////////////////////////////////////////////////

static NSString * formatTTimeInterval(CGFloat seconds, BOOL isLeft)
{
...
@interface HudTView : UIView
@end

@implementation HudTView
...
@interface kxTubeViewController ()
{
...
@implementation kxTubeViewController
@synthesize  downloadable, documentPath, videoPath, item;
                               ...
                               parameters: (NSDictionary *) parameters
{
    ...
    return [[kxTubeViewController alloc] initWithContentPath: videoItem parameters: parameters downloadable:YES];
                ...
                parameters: (NSDictionary *) parameters
{
    ...
    if (self) {
        ...
        __weak kxTubeViewController *weakSelf = self;
        ...
        decoder.interruptCallback = ^BOOL(){
            ...
            __strong kxTubeViewController *strongSelf = weakSelf;
...
- (id) initWithContentPath: (videoItem *) videoItem
                parameters: (NSDictionary *) parameters downloadable:(BOOL)can
{
    NSAssert(videoItem.videoid.length > 0, @"empty path");
...
-(void)setDecoder{
    ...
    if ([videoPath.pathExtension isEqualToString:@"wmv"]){
        parameters[KxTMovieParameterMinBufferedDuration] = @(5.0);
    }
    
    
    // disable deinterlacing for iPhone, because it's complex operation can cause stuttering
    if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone){
        parameters[KxTMovieParameterDisableDeinterlacing] = @(YES);
    }
    
    parameters[KxTMovieParameterMinBufferedDuration] = @(1.);
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        ...
        NSError *error = nil;
        [decoder openFile:videoPath error:&error];
...
- (void)loadView
{
    ...
    if ([library videoAtPathIsCompatibleWithSavedPhotosAlbum:[NSURL URLWithString:videoPath]]) {
...
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
	...
	if ([linkVideo isEqualToString:@""] == NO)
	{
        ...
        if ([[dh platformString] isEqualToString:@"iPhone 4"]) {
            __block NSString* k=@"0";
            __block NSString* l=nil;
            [links enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop){
                if (![obj isEqualToString:linkVideo]&& [key integerValue]>=[k integerValue]) {
                    l=obj;
                    k=key;
                }
            }];
...
{
    UIActionSheet *actionSheet = [[UIActionSheet alloc] initWithTitle:NSLocalizedString(@"Select video resolution", nil) delegate:self cancelButtonTitle:nil
                                               destructiveButtonTitle:nil otherButtonTitles:nil, nil];
    //-----------------------------------------------------------------------------------------------------------------------------------------
    for (NSString *itemx in [links allKeys]) [actionSheet addButtonWithTitle:itemx];
    //-----------------------------------------------------------------------------------------------------------------------------------------
    actionSheet.cancelButtonIndex = [links count]; [actionSheet addButtonWithTitle:NSLocalizedString(@"Cancel", nil) ];
    //-----------------------------------------------------------------------------------------------------------------------------------------
    [actionSheet showFromRect:sender.frame inView:self.view animated:YES];
    /*
    AppDelegate* app=(AppDelegate*)[[UIApplication sharedApplication] delegate];
    
    Download* t=[[Download alloc] initWith:videoPath withDelegate:app.downloading andDestination:documentPath];
    if (app.downloading.list==nil) {
        app.downloading.list=[[NSMutableArray alloc] initWithCapacity:0];
    }
    [app.downloading.list addObject:t];
    [app.downloading downloadList];
    [app.downloading.table reloadData];
    [sender setTitle:[NSString stringWithFormat:@"%d",app.downloading.list.count] forState:UIControlStateNormal];
    [sender setTitle:[NSString stringWithFormat:@"%d",app.downloading.list.count] forState:UIControlStateHighlighted];
    sender.enabled=NO;
     */
...
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
	...
	//---------------------------------------------------------------------------------------------------------------------------------------------
	if ([buttonTitle isEqualToString:NSLocalizedString(@"Cancel", nil) ] == NO)
	{
               ...
               withError: (NSError *) error
{
    ...
    if (!error && decoder) {
        ...
        if (_parameters.count) {
            ...
            val = [_parameters valueForKey: KxTMovieParameterMinBufferedDuration];
            if ([val isKindOfClass:[NSNumber class]])
                _minBufferedDuration = [val floatValue];
            
            val = [_parameters valueForKey: KxTMovieParameterMaxBufferedDuration];
            if ([val isKindOfClass:[NSNumber class]])
                _maxBufferedDuration = [val floatValue];
            
            val = [_parameters valueForKey: KxTMovieParameterDisableDeinterlacing];
            if ([val isKindOfClass:[NSNumber class]])
...
- (void) updateHUD
{
    ...
    _progressLabel.text = formatTTimeInterval(position, NO);
    [_progressButton performSelectorOnMainThread:@selector(setTitle:) withObject:formatTTimeInterval(position, NO) waitUntilDone:NO];
    if (_decoder.duration != MAXFLOAT)
        _leftLabel.text = formatTTimeInterval(duration - position, YES);
